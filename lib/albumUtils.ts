...
export async function createAlbumPage(imageData: Record<string, string>): Promise<string> {
    ...
    // Define the canonical order of decades to ensure correct layout.
    const orderedDecades = ['1950s', '1960s', '1970s', '1980s', '1990s', '2000s'];

    const imagesToLoad = orderedDecades
        .map(decade => ({ decade, url: imageData[decade] }))
        .filter(item => item.url); // Ensure we only process decades that have a URL.

    const loadedImages = await Promise.all(
        imagesToLoad.map(item => loadImage(item.url))
    );

    const imagesWithDecades = imagesToLoad.map((item, index) => ({
        decade: item.decade,
        img: loadedImages[index],
    }));

    // 4. Define grid layout and draw each polaroid
    // Reduced padding and increased image size within cells for larger photos.
    const grid = { cols: 2, rows: 3, padding: 80 };
    ...
    // Calculate polaroid dimensions to fit inside the grid cell with a smaller margin
    const polaroidAspectRatio = 1.2; // height is 1.2 times width
    const maxPolaroidWidth = cellWidth * 0.95;
    const maxPolaroidHeight = cellHeight * 0.95;
    ...
    reversedImages.forEach(({ decade, img }, reversedIndex) => {
        ...
        ctx.fillStyle = '#222';
        ctx.font = `65px 'Permanent Marker', cursive`; // Slightly larger font
        ctx.textAlign = 'center';
