...
const GHOST_POLAROIDS_CONFIG = [
  ...
  { initial: { x: "-200%", y: "0%", rotate: -15 }, transition: { delay: 0.1 } },
  { initial: { x: "200%", y: "50%", rotate: 15 }, transition: { delay: 0.7 } },
  { initial: { x: "-50%", y: "-180%", rotate: 5 }, transition: { delay: 0.9 } },
  { initial: { x: "50%", y: "180%", rotate: -5 }, transition: { delay: 1.0 } },
];
...
function App() {
    ...
    const handleRegenerateDecade = async (decade: string) => {
        ...
        try {
            // New prompt for regeneration to encourage variety
            const prompt = `Generate a new and different version. Reimagine the person in this photo in the style of the ${decade}. Explore alternative clothing, hairstyles, and settings appropriate for that era. The output must be a photorealistic image showing the person clearly.`;
            const resultUrl = await generateDecadeImage(uploadedImage, prompt);
            setGeneratedImages(prev => ({
                ...prev,
                [decade]: { status: 'done', url: resultUrl },
